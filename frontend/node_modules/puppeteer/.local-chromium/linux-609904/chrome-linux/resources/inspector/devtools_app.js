...
_appendCheckbox(label,subtitle,setting){const checkboxLabel=UI.CheckboxLabel.create(label,false,subtitle);UI.SettingsUI.bindCheckbox(checkboxLabel.checkboxElement,setting);this.contentElement.appendChild(checkboxLabel);}};;InspectorMain.RequestAppBannerActionDelegate=class{handleAction(context,actionId){const target=SDK.targetManager.mainTarget();if(target&&target.type()===SDK.Target.Type.Frame){target.pageAgent().requestAppBanner();Common.console.show();}
return true;}};;InspectorMain.InspectorMain=class extends Common.Object{run(){SDK.initMainConnection(()=>{const type=Runtime.queryParam('v8only')?SDK.Target.Type.Node:SDK.Target.Type.Frame;const target=SDK.targetManager.createTarget('main',Common.UIString('Main'),type,null);target.runtimeAgent().runIfWaitingForDebugger();},Components.TargetDetachedDialog.webSocketConnectionLost);new InspectorMain.InspectedNodeRevealer();new InspectorMain.SourcesPanelIndicator();new InspectorMain.BackendSettingsSync();new MobileThrottling.NetworkPanelIndicator();InspectorFrontendHost.events.addEventListener(InspectorFrontendHostAPI.Events.ReloadInspectedPage,event=>{const hard=(event.data);SDK.ResourceTreeModel.reloadAllPages(hard);});}};InspectorMain.ReloadActionDelegate=class{handleAction(context,actionId){switch(actionId){case'inspector_main.reload':SDK.ResourceTreeModel.reloadAllPages(false);return true;case'inspector_main.hard-reload':SDK.ResourceTreeModel.reloadAllPages(true);return true;}
